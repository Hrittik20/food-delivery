'use client'
import { Button, Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, Slider } from '@nextui-org/react';
import React, { useState } from 'react';
import { Label } from '@radix-ui/react-label';
import { Input } from '@nextui-org/react';
import { cn } from '@nextui-org/react';
import {
  IconBrandGithub,
  IconBrandGoogle,
} from "@tabler/icons-react";
import axios from 'axios';
import { useRouter } from 'next/navigation';

interface UserCredentials {
  fullName: string,
  password: string,
  email: string,
  address: string,
  birthDate: Date,
  gender: string,
  phoneNumber: string
}


const page = () => {
  const router = useRouter();

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();

      const formData = new FormData(e.currentTarget);
      const formValues = Object.fromEntries(formData.entries());

      const fullName = `${formValues.firstname} ${formValues.lastname}`;
      const gender = selectedGender;
      const date = new Date(formValues.birthDate as string);

      const credentials: UserCredentials = {
          fullName,
          password: formValues.password as string,
          email: formValues.email as string,
          address: formValues.address as string,
          birthDate: date,
          gender,
          phoneNumber: formValues.phoneNumber as string
      };

      const response = await fetch(`https://food-delivery.int.kreosoft.space/api/account/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(credentials),
      });

      console.log('Response:', response);
      if (!response.ok) {
        throw new Error('Invalid login credentials');
      }
      const user = await response.json();

      if (user) {
        console.log(user);
        localStorage.setItem('token', user.token);
        router.push('/profile');
      } else {
        throw new Error('Invalid login credentials');
      }
  };

  const [selectedGender, setSelectedGender] = useState('Select Gender');

  // Step 3: Update state on selection
  const handleSelect = (value: string) => {
      setSelectedGender(value);
  };

  return (
    <div className="max-w-md w-full mx-auto rounded-none md:rounded-2xl p-4 md:p-8 lg:p-14 shadow-input bg-white dark:bg-black">
      <h2 className="font-bold text-xl text-neutral-800 dark:text-neutral-200">
        Welcome to DeliveryEats
      </h2>
      <p className="text-neutral-600 text-sm max-w-sm mt-2 dark:text-neutral-300">
        The best obesity at your fingertips!
      </p>
 
      <form className="my-8" onSubmit={handleSubmit}>
        <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-4">
          <LabelInputContainer>
            <Label htmlFor="firstname">First name</Label>
            <Input id="firstname" placeholder="Tyler" type="text" name='firstname'/>
          </LabelInputContainer>
          <LabelInputContainer>
            <Label htmlFor="lastname">Last name</Label>
            <Input id="lastname" placeholder="Durden" type="text" name='lastname'/>
          </LabelInputContainer>
        </div>
        <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-4" >
            <LabelInputContainer>
            <Label htmlFor="gender">Gender</Label>
            <Dropdown>
                <DropdownTrigger>
                <Button variant="flat" style={{ display: 'flex', justifyContent: 'flex-start' }} className='w-auto'>{selectedGender}</Button>
                </DropdownTrigger>
                <DropdownMenu aria-label="Gender Options" variant='solid' className='bg-white dark:bg-black w-auto text-left'>
                <DropdownItem key="male" value="option1"  onClick={() => handleSelect('Male')} className='w-full'>Male</DropdownItem>
                <DropdownItem key="female" value="option2"  onClick={() => handleSelect('Female')} className='w-full'>Female</DropdownItem>
                </DropdownMenu>
            </Dropdown>
            </LabelInputContainer>
            <LabelInputContainer>
                <Label htmlFor="dob">Date of Birth</Label>
                <Input id="dob" placeholder="30-02-2002" type="date" name='birthDate' />
            </LabelInputContainer>
        </div>
        <LabelInputContainer className="mb-8">
          <Label htmlFor="address">Address</Label>
          <Input
            id="address"
            placeholder="Harat's Pub, Tomsk, Russia"
            type="address"
            name='address'
          />
        </LabelInputContainer>
        <LabelInputContainer className="mb-8">
          <Label htmlFor="address">Phone Number</Label>
          <Input
            id="phoneNumber"
            placeholder="+7 (xxx) xxx-xx-xx"
            type="string"
            name='phoneNumber'
          />
        </LabelInputContainer>
        <LabelInputContainer className="mb-4">
          <Label htmlFor="email">Email Address</Label>
          <Input id="email" placeholder="projectmayhem@fc.com" type="email" name='email' />
        </LabelInputContainer>
        <LabelInputContainer className="mb-4">
          <Label htmlFor="password">Password</Label>
          <Input id="password" placeholder="••••••••" type="password" name='password'/>
        </LabelInputContainer>
 
        <button
          className="bg-gradient-to-br relative group/btn from-black dark:from-zinc-900 dark:to-zinc-900 to-neutral-600 block dark:bg-zinc-800 w-full text-white rounded-md h-10 font-medium shadow-[0px_1px_0px_0px_#ffffff40_inset,0px_-1px_0px_0px_#ffffff40_inset] dark:shadow-[0px_1px_0px_0px_var(--zinc-800)_inset,0px_-1px_0px_0px_var(--zinc-800)_inset]"
          type="submit"
        >
          Sign up &rarr;
          <BottomGradient />
        </button>
 
        <div className="bg-gradient-to-r from-transparent via-neutral-300 dark:via-neutral-700 to-transparent my-8 h-[1px] w-full" />
 
        <div className="flex flex-col space-y-4">
          <button
            className=" relative group/btn flex space-x-2 items-center justify-start px-4 w-full text-black rounded-md h-10 font-medium shadow-input bg-gray-50 dark:bg-zinc-900 dark:shadow-[0px_0px_1px_1px_var(--neutral-800)]"
            type="submit"
          >
            <IconBrandGithub className="h-4 w-4 text-neutral-800 dark:text-neutral-300" />
            <span className="text-neutral-700 dark:text-neutral-300 text-sm">
              GitHub
            </span>
            <BottomGradient />
          </button>
          <button
            className=" relative group/btn flex space-x-2 items-center justify-start px-4 w-full text-black rounded-md h-10 font-medium shadow-input bg-gray-50 dark:bg-zinc-900 dark:shadow-[0px_0px_1px_1px_var(--neutral-800)]"
            type="submit"
          >
            <IconBrandGoogle className="h-4 w-4 text-neutral-800 dark:text-neutral-300" />
            <span className="text-neutral-700 dark:text-neutral-300 text-sm">
              Google
            </span>
            <BottomGradient />
          </button>
        </div>
      </form>
    </div>
  )
}

const BottomGradient = () => {
    return (
      <>
        <span className="group-hover/btn:opacity-100 block transition duration-500 opacity-0 absolute h-px w-full -bottom-px inset-x-0 bg-gradient-to-r from-transparent via-cyan-500 to-transparent" />
        <span className="group-hover/btn:opacity-100 blur-sm block transition duration-500 opacity-0 absolute h-px w-1/2 mx-auto -bottom-px inset-x-10 bg-gradient-to-r from-transparent via-indigo-500 to-transparent" />
      </>
    );
  };
   
  const LabelInputContainer = ({
    children,
    className,
  }: {
    children: React.ReactNode;
    className?: string;
  }) => {
    return (
      <div className={cn("flex flex-col space-y-2 w-full", className)}>
        {children}
      </div>
    );
  };

export default page